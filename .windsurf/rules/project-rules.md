...
* Always read the 'rh-planning.md' markdown file in the `planning` directory at the start of a new conversation to understand the project's architecture, goals, style, and constraints.
* Check `rh-tasks.md` before starting a new task. If the task isn’t listed, add it with a brief description and today's date. If that file doesn’t exist prompt me to create one.
...
* Don’t use promises.
* Deployment will be on **Firebase**.
* Any backend database/compute requirements will use **Firestore/Cloud Functions**.
